...
export class LimitedLiveMatch implements OnInit {
  ...
  firstInningsOutPlayerIds: string[] = [];
  ...
  get currentBowlingTeamName(): 'A' | 'B' {
    return this.currentBattingTeamName === 'A' ? 'B' : 'A';
  }
  ...
  saveSnapshot() {
    this.matchHistory.push(
      structuredClone({
        ...
        firstInningsOutPlayerIds: this.firstInningsOutPlayerIds,
  ...
  totalExtras(wide: number, noBall: number) {
  ...
  getYetToBatFirstInningsPlayers() {
    return (this.firstInningsBattingTeam ?? []).filter(player =>
      ...
      !this.firstInningsOutPlayerIds.includes(player.id!)
    );
  }
